...
The last employed technique involved a more traditional approach using FTK Imager, XRY, and Autospy. We attempted to connect each device to FTK Imager, but only the Garmin was recognized. While we were not expecting any device to be recongized, we were able to gather a lot of data from the Garmin. We then used XRY to create an image of the Android phone that had been connected to each IoT device. In examining the image of the phone, we were able to pull some of the data that had been generated by the IoT devices.   
